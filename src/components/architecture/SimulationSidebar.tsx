...
function AnnotationBadge({ annotation, index }: { annotation: { type: string; content: string; sourceRef: string }; index: number }) {
  const colorClass = ANNOTATION_COLORS[annotation.type as keyof typeof ANNOTATION_COLORS] || ANNOTATION_COLORS.STAGE
  const icon = ANNOTATION_ICONS[annotation.type as keyof typeof ANNOTATION_ICONS] || '📝'
