...
export function createRpcService(): RpcService {
  ...
  return {
    async submitTransaction(tx: Transaction, _commitment: CommitmentLevel = 'processed'): Promise<SubmitResult> {
      // Store transaction
    ...
    async simulateTransaction(_tx: Transaction): Promise<SimulationResult> {
      // Mock simulation
      return {
        success: true,
        logs: [
    ...
    async getBlock(slot: number, _commitment: CommitmentLevel = 'processed'): Promise<Block | null> {
      return blocks.get(slot) || null
    },

    async getRecentBlockhash(_commitment: CommitmentLevel = 'processed'): Promise<BlockhashResult> {
      return {
