...
- [x] T018 [US3] Add `rpc-api` node to src/components/architecture/data/components.ts (networking layer; client-facing submission entry: sendTransaction → push toward upcoming leaders via QUIC; send-transaction-service behavior); edge (rpc-api,quic-streamer) in connections.ts; retarget tour step 1; tests same commit
- [x] T019 [US3] Merge `gulf-stream` + `forwarding` into single node id `forwarding` named "Forwarding (Gulf Stream)" in src/components/architecture/data/components.ts per Clarifications: retire gulf-stream id + edge (gulf-stream,quic-streamer); move duplicated leader-schedule sub-content under epoch-schedule; enforce FR-014 bans (no ahead-of-time execution, no forwarder-stake priority; blockhash-expiry framing); edges (sig-verify,forwarding),(forwarding,quic-streamer); tests same commit
- [x] T020 [US3] Add `cluster-info-vote-listener` node (consensus layer) to src/components/architecture/data/components.ts: polls cluster votes, CPU-verifies, tracks VoteTracker, fires optimistic/duplicate-confirmation thresholds, feeds verified gossip votes into banking when leader; refs
- [x] T021 [US3] Add `voting-service` node (consensus layer) to src/components/architecture/data/components.ts: receives ReplayStage/Tower vote ops, persists tower storage, publishes own votes outward; refs
- [x] T022 [US3] Correct propagation-side components in src/components/architecture/data/components.ts as six sequential one-component commits (FR-021, FR-024), each with refs + report row + suite/tsc gate before committing (FR-022):
...
- [x] T025 [US4] Manual citation spot-check per specs/003-agave-accuracy-audit/quickstart.md §4: sample ≥10 links across different components/tour steps; record results in report.md methodology appendix
