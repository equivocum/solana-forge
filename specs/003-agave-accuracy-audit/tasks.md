...
- [ ] T022 [US3] Correct propagation-side contents in src/components/architecture/data/components.ts: `broadcast` (Merkle-signed shreds, chained_merkle_root, FEC 32:32, header anatomy), `shred-sig-verify` (Merkle-root verification vs slot's scheduled leader pubkey, LRU cache), `window-service` (Bloom dedup, erasure recovery + retransmit of recovered data shreds, duplicate-conflict detection), `turbine` (stake-weighted trees, data/coding interleave), `quic-streamer` (three endpoints solQuicTpu/solQuicTpuFwd/solQuicTVo, stake-weighted QoS); refs; report rows
