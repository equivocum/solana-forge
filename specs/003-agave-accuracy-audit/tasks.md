...
**Organization**: Tasks grouped by user story; each implementation task = one conventional commit on branch `003-agave-accuracy-audit` (FR-020/021); every story ends with a verification gate (FR-022).
...
- [ ] T001 Run baseline verification gates on branch 003-agave-accuracy-audit: `pnpm test` and `npx tsc --noEmit` must both pass with zero failures before any file is modified
...
- [ ] T002 Pin exact line anchors against tag v4.2.1 for all core claims in specs/003-agave-accuracy-audit/research.md (D-2 through D-12): verify each permalink resolves and record final `#L<start>-L<end>` anchors into a working notes section of specs/003-agave-accuracy-audit/report.md
...
- [ ] T022 [US3] Correct propagation-side contents in src/components/architecture/data/components.ts: `broadcast` (Merkle-signed shreds, chained_merkle_root, FEC 32:32, header anatomy), `shred-sig-verify` (Merkle-root verification vs slot's scheduled leader pubkey, LRU cache), `window-service` (Bloom dedup, erasure recovery + retransmit of recovered data shreds, duplicate-conflict detection), `turbine` (stake-weighted trees, data/coding interleave), `quic-streamer` (three endpoints solQuicTpu/solQuicTpuFwd/solQuicTVo, stake-weighted QoS), `accounts-db` and `bank` (execution writes deltas to bank state; durable persistence is asynchronous — stored at freeze, squashed/consolidated around root advancement by AccountsBackgroundService, hardened via snapshots); refs; report rows
...
- [ ] T027 Update README.md architecture summary bullets (TPU/TVU pipeline descriptions, component count, vote-loop mention) to match corrected model; run full specs/003-agave-accuracy-audit/quickstart.md validation end-to-end; open PR from 003-agave-accuracy-audit to main referencing constitution Git Flow (issue/self-review/squash-merge)
...
- **Commit discipline**: One logical correction per conventional commit (FR-021); structural data changes always pair with test updates in the same commit (FR-018); merge to main solely via PR after all gates (FR-020/023)
...
- Success criteria mapping: SC-001→T010/T016/T023; SC-002→T024/T025; SC-003→T026; SC-004→T017; SC-005→gates T011/T017/T023/T027; SC-006→T026
